...
class DS1307Class():
    ...
    def _write(self, register, data):
        ...
        c = "i2cset -y 1 0x%x 0x%x 0x%x" % (self._addr, register, data)
        s = subprocess.check_output(c, shell=True)


    def _read(self, register):
        c = "i2cget -y 1 0x%x 0x%x" % (self._addr, register)
        s = subprocess.check_output(c, shell=True)
        returndata = int(s, 16)
    ...
    def read_srt_sys_date(self):
        """Return a string such as '20YY-DD-MM- HH:MM:SS'.
        """
        return '20%02d-%02d-%02d %02d:%02d:%02d' % (self._read_year(),
                self._read_month(), self._read_date(), self._read_hours(),
                self._read_minutes(), self._read_seconds())
